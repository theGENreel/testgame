...
def update(screen, camera: Camera, game_window, info_window):
    key = screen.getch()
    camera.draw(game_window)
    camera.draw_info(info_window)
    camera.map.input(key)
...
def main(screen):
    ...
    while True:
        ...
        game_window.clear()
        info_window.clear()
        update(screen, camera, game_window, info_window)
